 ...
 *
 * GET  /channels  — proxies WhatsAppClient::listOwnedChannels; 502 on transport failure.
 * POST /ui/send   — the human send path. Guards chatId + text SERVER-SIDE before any
 *                   engine call; routes through WhatsAppClient::sendText on success.
 *                   NOT gated (no X-Internal-Key).
 * POST /send      — machine-facing, gated send path. Requires X-Internal-Key header;
 *                   401 is returned BEFORE guards and BEFORE any engine call when the
 *                   key is missing or wrong. Shares the same guard + delivery path as
 *                   /ui/send — exactly one engine delivery path in the codebase.
 */
final class ChannelController extends AbstractController
{
    public function __construct(
        private readonly WhatsAppClient $engine,
    ...
    {
        try {
            return new JsonResponse($this->engine->listOwnedChannels());
        } catch (\Throwable $e) {
    ...
    {
        // 401 fires BEFORE guards and BEFORE any engine call.
    ...
    private function deliver(string $chatId, string $text, ?array $preview = null): JsonResponse
    {
        ...
        $result = $this->engine->sendText($chatId, $text, $preview);

        if (!$result['ok']) {
            return new JsonResponse(
                ['error' => "Engine returned {$result['status']}"],
