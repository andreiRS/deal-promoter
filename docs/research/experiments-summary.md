...
batchable deal *discovery* + the free **Pre-filter**; the **Creators API** supplies the
**Live Snapshot** (live price + facts) and the affiliate link. The non-negotiable rule both
halves point at: **never publish a price you have not just re-confirmed with a fresh Live
Snapshot.**

Full flow: `raw candidate → free Pre-filter (Criteria + Outlier Guards) → optional deep
stats check → Creators Live Snapshot → Deal Gate → publish`.

---

## Keepa API — discovery + Pre-filter (exp 01–05)
...
- **Landmine:** sorting by `deltaPercent` (sortType 4) floats **Price-Outlier garbage** to
...
### 04 · Which `stats` bounds separate real deals from Price Outliers?
The "claimed vs verified drop" idea is **dead** — Keepa derives both from the same avg90,
so divergence is always 0 (drop it). What works structurally are the **Outlier Guards**:
`spike-polluted-baseline` (avg90 > 3×avg30), `no-demand` (salesRankDrops90 < 1),
`abs-price-floor` (< €2), `absurd-claim` (>97%). Of the top-25 by %-drop, **only 2
survived**. Crucially, **all four Outlier Guards run on the cheap `/deal` payload alone** —
no `/product` needed to pre-filter.
- **Residual risk:** a *persistently-wrong Reference Price* (long OOS + third-party
  gouging, then an Amazon restock at the real price) defeats every average-based guard. The
  Live Snapshot is the only real backstop.

### 05 · Does the end-to-end funnel run sustainably?
Yes. **150 deals → ~26 surviving candidates (free Pre-filter, 0 API calls) → 26 after deep
re-check.** Full pass = **31 tokens** (5 deal + 26 product) ≈ 1.55 min of refill → **~39
passes/hour**, sustainable on the entry tier. The deep `/product` stage rejected **0/26**
here — it's **optional confirmation**, its only unique signal being `outOfStockPercentage90`.
Treat final surviving candidates as **inputs for a Live Snapshot, never publish-ready.**
Compare `deal.avg*` to `stats.*` with a **tolerance, never `===`** (same value, rounded;
within 1 cent, not bit-identical). Pre-filter token savings are feed-dependent — they appear
on a *filtered* production query, not a raw worst-case sortType=4 page.

---

## Amazon Creators API — Live Snapshot + affiliate link (exp 06–08)
...
vs the Keepa Reference Price**; savings%/dealDetails are corroboration and post copy, not the
gate. Corrections to the brief:
...
## Full pipeline — discovery + Live Snapshot wired together (exp 09)
...
took 150 deals → 17 free-Pre-filter surviving candidates → top-10 by score → Deal Gate on the
live Creators snapshot → a branded HTML table. The handoff works and the affiliate
`detailPageURL` (with `tag=`) renders.
- **Load-bearing finding:** the Live Snapshot confirms **Price Validity** (a price is *real and
  buyable*), NOT **Discount Magnitude** (the size of the discount). The gate's "live price vs
  Keepa `avg90`" published fake 80%+ drops: one item read 80% off avg90 but Amazon's own
  `WAS_PRICE` said 12%; another read 84% off while Amazon flagged no deal at all. Keepa's avg90
  is the persistently-polluted Reference Price (long OOS + third-party gouging) that exp04/05
  warned about, and the Live Snapshot cannot un-pollute history.
...
- **No deep `/product?stats` stage:** dropped on purpose. It would not catch the above (the Live
  Snapshot can't un-pollute history; the magnitude fix lives in the Creators fields), and it
...
- **Keepa is cheap, batchable discovery** (5/page + 1/survivor, ~39 passes/hr) — but its
  averages are **Price-Outlier-polluted**, and no average-based Outlier Guard can catch a
  persistently-wrong Reference Price.
- **The Live Snapshot is the live backstop** — authoritative current price + Amazon's own deal
  claim + the ready-to-post tagged affiliate URL.
- **Never publish a price you have not just re-confirmed with a fresh Live Snapshot.** That
